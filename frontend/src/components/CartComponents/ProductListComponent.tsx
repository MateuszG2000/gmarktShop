...
function ProductListComponent({ moreData, product }: { moreData: boolean; product: ICartProduct }) {
  ...
  const handleQuantity = debounce((quantity: number) => {
    ...
    dispatch(cartActions.updateQuantity({ quantity: quantity, id: product._id }));
  }, 400);
  const handleRemove = () => {
  ...
  return (
    <div className={css.productListComponent}>
      ...
      {isMobile && (
        <>
          <select onChange={(e) => handleQuantity(e.target.value)} defaultValue={product.quantity}>
      ...
      {moreData && (
        <button aria-label="usuń" onClick={handleRemove} className={css.trash}>
          <BsFillTrash3Fill />
        </button>
      )}
    </div>
